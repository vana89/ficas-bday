...
// Escape room je rezervisan za 17:00.
function applyBookedSchedule(){
  const rows=document.querySelectorAll('.event-card .event-row');
  const start=rows.length?rows[rows.length-1].querySelector('.event-value'):null;
...
function bookedCountdown(){const el=document.getElementById('countdown');if(!el)return;const target=new Date('2026-09-26T17:00:00+02:00');const d=target-new Date();if(d<=0){el.textContent='Vreme je za rođendan! 🎉';return;}const days=Math.floor(d/86400000),hrs=Math.floor((d%86400000)/3600000);el.textContent=`Još ${days} dana i ${hrs} h do početka`;}
applyBookedSchedule();bookedCountdown();setInterval(bookedCountdown,10000);
