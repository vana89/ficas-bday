...
function ensureSyncUi(){if(document.getElementById('syncStatus'))return;const note=document.querySelector('#guests .small-note');if(!note)return;const wrap=document.createElement('div');wrap.style.cssText='display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:10px';const uki=STATE_ID==='uki-2026';const border=uki?'#c9e0e8':'#c7e3de';const bg=uki?'#e8f3f7':'linear-gradient(135deg,#d8efeb,#bfe3dd)';const color=uki?'#315f70':'#163f3b';wrap.innerHTML=`<button id="syncSaveBtn" type="button" style="border:1px solid ${border};background:${bg};color:${color};border-radius:12px;padding:8px 12px;font:inherit;font-weight:800;cursor:pointer">Sačuvaj</button><span id="syncStatus" style="font-size:12px;color:#667486">Spremno</span>`;note.after(wrap);document.getElementById('syncSaveBtn').addEventListener('click',saveShared);}
